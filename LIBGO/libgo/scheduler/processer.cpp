...
void Processer::Process()
{
    ...
    EnvUtils::SetThreadAffinite(5, 6);
    ...
    std::cout << "QueueSize:" <<  RunnableSize() << std::endl;
#if ENABLE_DEBUGGER
        ...
        while (runningTask_ && !scheduler_->IsStop()) {
            runningTask_->state_ = TaskState::runnable;     // it is in the runn queue
...
{
    if (!markSwitch_ || markSwitch_ != switchCount_) {
        return false;
    }
